...
{
$id=$_GET['id'];
$qry=mysql_query("SELECT * FROM galery WHERE id=$id", $con);
if(!$qry)
{
die("Query Failed: ". mysql_error());
}

                /* Fetching data from the field "image" */
...
//echo $_GET['cat'];
$cat=$_GET['cat'];
$qry=mysql_query("SELECT * FROM galery WHERE category='$cat' order by galery.id DESC LIMIT 0, 5", $con);
if(!$qry)
{
die("Query Failed: ". mysql_error());
}

                /* menyamakan data dari field tittle */
while($row=mysql_fetch_array($qry))
{
...
{
$id=$_GET['id'];
$qry=mysql_query("SELECT category FROM galery WHERE id='$id'", $con);
if(!$qry)
{
die("Query Failed: ". mysql_error());
}
...
<div class="fl_right">
      ...
      <ul>
        ...
        <li class="last">
          <?php
/*
menampilkan data terakhir dari field "galery"
*/
$qry=mysql_query("SELECT * FROM galery WHERE category='berita' order by galery.id DESC LIMIT 0, 4", $con);
if(!$qry)
{
die("Query Failed: ". mysql_error());
}

/* mengambil dan menampilkan databse table "galery" */
/*
...
echo "<p>".substr($row['contents'],0,100)."<a href=galery.php?id=".$row['id']." > Read more</a></p>";
}
?>
        </li>
